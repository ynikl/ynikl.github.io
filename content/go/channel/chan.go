...
type hchan struct {
	qcount   uint           // total data in the queue # 环形数据的buffer个数
	dataqsiz uint           // size of the circular queue #
	buf      unsafe.Pointer // points to an array of dataqsiz elements # 环形数据开始地址
	elemsize uint16         // # channel 元素大小
	closed   uint32         // # 是否已经关闭 0未关闭, 非0关闭
	elemtype *_type         // element type # channel 元素的类型
	sendx    uint           // send index  # 环形数据中, 发送数据存储的下标
	recvx    uint           // receive index # 环形数据中, 接受数据获取数据的下标
	recvq    waitq          // list of recv waiters # 挂在 channel 等待获取数据的 Groutine 列表
...
// chanbuf(c, i) is pointer to the i'th slot in the buffer. # 获取 第i个元素地址
